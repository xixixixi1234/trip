...
function GuestReviews({ hotelId }) {
  ...
  const load = () => {
    let alive = true;
    setState({ status: "loading", items: [] }); setShown(5);
    fetchJson(`/api/hotels/${encodeURIComponent(hotelId)}/reviews`)
      .then(rs => { if (alive) setState({ status: "ok", items: (rs || []).filter(r => r.source === "quote" && r.text) }); })
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: 20, marginBottom: 20 }}>
      ...
      {state.status === "ok" && items.length > shown && (
        <button type="button" onClick={() => setShown(n => n + 10)} className="wp-btn wp-ghost" style={{ marginTop: 6, background: C.card, border: `1px solid ${C.line}`, color: C.ink, fontWeight: 600, fontSize: 14, padding: "9px 16px", borderRadius: 99, minHeight: 40 }}>
          Show more reviews ({items.length - shown} left)
        </button>
      )}
    </div>
  );
