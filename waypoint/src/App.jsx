...
function CityPage({ cityKey, onBack, onOpen, votes, favs, cities, hotels: allHotels }) {
  ...
  // trackpad horizontal swipe to change pages (left swipe = prev, right swipe = next)
  useEffect(() => {
    let acc = 0, cooldown = false;
    const onWheel = (e) => {
      if (Math.abs(e.deltaX) <= Math.abs(e.deltaY)) return; // vertical scroll -> ignore
      e.preventDefault();
      if (cooldown) return;
      acc += e.deltaX;
      if (acc > 90) {            // swipe left (content moves left) -> previous page
        acc = 0; cooldown = true; setTimeout(() => cooldown = false, 500);
        setPage(p => Math.max(1, p - 1));
      } else if (acc < -90) {    // swipe right -> next page
        acc = 0; cooldown = true; setTimeout(() => cooldown = false, 500);
