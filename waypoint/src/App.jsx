        ...
        <div style={{ padding: "22px 24px" }}>
          ...
          {votes && (
            <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", paddingTop: 16, borderTop: `1px solid ${C.line}` }}>
              ...
              {favs && (
                <button onClick={() => favs.toggleHotel(listing.id)} style={{
                  display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer",
                  fontSize: 14, fontWeight: 600, fontFamily: "'Roboto', sans-serif",
                  padding: "8px 14px", borderRadius: 99,
                  border: `1px solid ${favs.hotelFavs.has(listing.id) ? C.buoy : C.line}`,
                  background: favs.hotelFavs.has(listing.id) ? C.buoy : C.card,
                  color: favs.hotelFavs.has(listing.id) ? "#fff" : C.inkSoft,
                }}>
                  {favs.hotelFavs.has(listing.id) ? "♥ 已收藏" : "♡ 收藏酒店"}
                </button>
              )}
            </div>
          )}
    ...
    <div ref={rowRef} onClick={onOpen} style={{
      ...
      <div style={{ padding: "14px 18px", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          ...
          {favs && (
            <button onClick={e => { e.stopPropagation(); favs.toggleHotel(l.id); }} title={faved ? "取消收藏" : "收藏该酒店"}
              style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, lineHeight: 1, color: faved ? C.buoy : C.line, padding: 0 }}>
              {faved ? "♥" : "♡"}
            </button>
          )}
        </div>
    ...
    <div style={{
      ...
      <div style={{ background: C.card, borderRadius: 16, padding: "30px 28px", maxWidth: 420, width: "100%", boxShadow: "0 24px 60px rgba(0,0,0,0.3)" }}>
        ...
        <p style={{ fontSize: 14.5, color: C.inkSoft, lineHeight: 1.6, margin: "0 0 18px" }}>
          请输入你的<b>被试编号</b>后开始。你在网站上的浏览与操作会以这个编号匿名记录，用于研究分析。
        </p>
...
export default function App() {
  ...
  // participant id (research tracking)
  ...
  const startSession = (id) => { setPid(id); Track.start(id); };
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <nav style={{ display: "flex", gap: 14, alignItems: "center" }}>
            ...
            <button onClick={favs.toggleSite} title="收藏本站" style={{
              display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer",
              fontSize: 13, fontWeight: 600, fontFamily: "'Roboto', sans-serif",
              padding: "7px 14px", borderRadius: 99,
              border: `1px solid ${favs.siteFav ? C.buoy : C.line}`,
              background: favs.siteFav ? C.buoy : C.card, color: favs.siteFav ? "#fff" : C.inkSoft,
            }}>
              {favs.siteFav ? "★ 已收藏本站" : "☆ 收藏本站"}
            </button>
          </nav>
        </div>
      </header>
