            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", paddingTop: 16, borderTop: `1px solid ${C.line}` }}>
              ...
              {favs && (
                <button onClick={() => favs.toggleHotel(listing.id)} style={{
                  ...
                  {favs.hotelFavs.has(listing.id) ? "Bookmarked" : "Bookmark hotel"}
                </button>
              )}
            </div>
          )}
    ...
    <div ref={rowRef} onClick={onOpen} style={{
      ...
      <div style={{ padding: "14px 18px", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          ...
          {favs && (
            <button onClick={e => { e.stopPropagation(); favs.toggleHotel(l.id); }} title={faved ? "Remove bookmark" : "Bookmark this hotel"}
              style={{
                flex: "0 0 auto", cursor: "pointer", fontFamily: "'Roboto', sans-serif",
                fontSize: 12, fontWeight: 600, padding: "5px 11px", borderRadius: 99,
                border: `1px solid ${faved ? C.buoy : C.line}`,
                background: faved ? C.buoy : C.card, color: faved ? "#fff" : C.inkSoft,
              }}>
              {faved ? "Bookmarked" : "Bookmark"}
            </button>
          )}
        </div>
