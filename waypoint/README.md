...
- **4 城 × 100 家酒店**（London / New York / Paris / Rome），全部同时具备 AI 摘要、酒店描述、图片，**起始价 $80–220**（v4.1），按三档抽取：低 ≤3.7 / 中 3.8–4.1 / 高 ≥4.2，每档尽量 33 家（纽约、罗马低档不够就全取），档内按评分逐级轮询、同级优先评论多的。分布见 `data/rating_distribution.csv`。
...
**Dashboard**（后台第二个标签）：九张实时卡片——Participants、Hotel likes/dislikes（分搜索页/详情页）、Review likes、Time on site、Most liked/disliked hotels、Review engagement（评论区总时长/平均深度）、AI conditions 分布、Latest participants。**卡片可拖拽排序**（触屏用 ‹ › 按钮），顶部勾选显示/隐藏；布局存在本设备浏览器里；栅格自适应屏幕宽度（手机单列、宽屏多列）；每 20 秒自动刷新。

## v4.3：评论区追踪 / 评论点赞 / 展示方式设置

**新追踪**（每个被试 × 每家酒店）：评论区在屏幕上的停留时间（reviews_dwell）、滑到了第几条（reviews_scrolled_to，取最深）、当时页面上一共有几条（reviews_shown_total）。后台 Participants 展开表新增 "Reviews seen"（如 7 / 20）和 "Reviews s" 列；`hotel_events.csv` 新增 4 列。

**评论点赞**：每条评论底部有 👍 Helpful / 👎 Not helpful（可在 Page elements 里关掉），再点一次取消，换点会覆盖。导出：Participation 页 → `review_votes.csv`（被试、评论、like/dislike、酒店、评论作者/评分/标题、时间）。

**展示方式**（Study settings → Guest reviews display，自动保存）：
- Reviews per hotel：每家最多放几条（0 = 导入的全放）
- Shown at first：首屏/每页几条
- Reading mode：Scrolling（滑到底自动加载下一批）或 Pages（‹ Previous / Next › 翻页，翻页会记入滑动深度）
例：一次全展 20 条 → 0 / 20 / Scrolling；先 5 条滑动加载 → 0 / 5 / Scrolling；每页 5 条翻页 → 0 / 5 / Pages。

## v4.2.1：后台自动保存

Edit hotel text、Reviews（已有评论）、Study settings（AI 开关、页面元素、欢迎文案）里的改动都会在**停止输入 2 秒后自动保存**，右侧显示 "Unsaved changes… → Saving… → Auto-saved 14:03:12"。"Save now" 按钮保留，想立刻保存就点。新增评论仍需点 Create；Hotel photos 的说明文字失焦即保存。
...
上传的照片存在**数据库**里（不是文件系统），所以接了 Postgres 后重新部署不会丢；内存模式下重启即丢。原始下载的那张不能在后台删，但可以换成别的做封面。建议上传前把图缩到 1600px 宽以内，页面加载快。

## 后台管理评论（v3.8）

后台新增「Reviews」页：选城市 → 选酒店 → 列出该酒店全部评论，每条可改（评论者、所在地、评分、评论日期、入住月份、出行类型、贡献数、有用票数、标题、正文）、可删、可删掉某张照片，右上角「+ Add review」新增评论。改动直接写库、前台立即生效。内存模式下重启会丢；Postgres 模式下 `data/reviews.csv` 没变就不会被覆盖（自动导入只在文件内容变化时触发）。添加新照片仍通过 `data/reviews.csv` + `npm run review-images`。

## 住客评论（v3.7，仿 TripAdvisor 排版）

详情页「Guest reviews」每条评论：头像 · **评论者** wrote a review Jul 2026 / 所在地 • N contributions • N helpful votes / 评分圆点 / 标题 / 正文 / 评论照片缩略图（点开大图）/ Date of stay: July 2026 / Trip type: Traveled with family。没有酒店回复、没有菜单按钮、没有点赞交互。一次 5 条 + Show more；没导入的酒店显示 "No guest reviews are available"。

**完整本地流程**
```bash
cd 项目目录
python3 scripts/filter_reviews.py 你导出的评论.csv --latest 20 --lang en     # 1. 每家最新 20 条英文 → data/reviews.csv
npm run review-images                                                            # 2. 下载评论照片 → public/images/reviews，头像 → public/images/avatars
git add data/reviews.csv public/images/reviews public/images/avatars              # 3. 随代码提交、推送
git commit -m "reviews" && git push
```
4. 部署即生效：服务器启动时**自动读取 `data/reviews.csv`**（v3.7.1）。内存模式每次启动都读；Postgres 模式只在文件内容变了才重新导（这样后台手改的评论不会每次重启被覆盖）。后台的 "Import guest reviews" 上传按钮只在想不重新部署就换评论时才用。

- `filter_reviews.py` 自动识别列（reviewer_name / reviewer_location / reviewer_contribution / reviewer_avatar / review_date / review_rating / review_title / review_text / trip_type / helpful_vote / picture_list / date_visited / review_language），日期按 日/月/年 解析后排序取最新；输出列 `hotel_id, author, location, rating, date, date_visited, title, text, trip_type, helpful, photos, language, contributions, avatar`。
- 照片和头像只在本地文件存在时显示，原始 URL 不发给浏览器；没有头像时显示首字母圆形。
- 评论照片按 550px 版本下载（TripAdvisor CDN 的 `photo-s`），头像按 250px（`photo-f`），拿不到就退回原图；每张几十 KB，20 条/家 ≈ 3,000 多张 ≈ 150–250 MB。想要原图：`IMAGE_SIZE=photo-o npm run review-images`。
- 文件里出现的酒店，其原有评论整体替换；RESEED 会清空评论表，之后需重新导入。
