...
CSV 里的 `image_url` 在 `npm run build` 时会自动下载到本地（`prebuild` 钩子 → `scripts/download_images.js`），所以 **Railway 部署时不用做任何额外操作**：`npm install → npm run build`（下载图片 + 打包前端）`→ npm start`。

- 存放位置：`public/images/hotels/<酒店id>.jpg`，由 `server.js` 直接以 `/images/hotels/...` 提供（带 7 天缓存头）。不进 git，也不会重复拷进 `dist/`。
- `/api/hotels` 返回时：有本地文件的酒店 `image` 指向本地路径，原始 URL 放在 `imageRemote`；前端本地加载失败会自动回退到原始 URL，再失败才显示渐变占位。
- 脚本永远不会让构建失败：某几张下载失败只会记录在 `public/images/hotels/failed.tsv`，这些酒店前端直接用原始 URL。
- 已下载的会跳过，重新部署只补缺的（Railway 每次重新构建是干净环境，会全部重下，399 张大约 1–2 分钟）。
- 手动命令：`npm run images`（补缺）、`node scripts/download_images.js --force`（全部重下）、`SKIP_IMAGE_DOWNLOAD=1 npm run build`（跳过）。
...
5. Railway 自动 `npm install → npm run build（含自动下载酒店图片）→ npm start`。构建日志里能看到 `Done. N downloaded …`。
...
├── scripts/
│   └── download_images.js   # 下载所有酒店 image_url 到 public/images/hotels（build 时自动跑）
├── public/images/hotels/    # 下载好的图片（自动生成，不进 git）
├── data/
