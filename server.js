...
app.get("/api/hotels", async (req, res) => {
  try { res.json(withLocalImages(await db.listHotels({ city: req.query.city }))); }
...
app.get("/api/settings/welcome", async (_req, res) => {
  try { res.json({ text: await db.getSetting("welcome", db.DEFAULT_WELCOME) }); }
  catch (e) { console.error(e); res.json({ text: db.DEFAULT_WELCOME }); }
});
app.post("/api/track/consent", async (req, res) => {
  const { pid } = req.body || {};
  if (!pid) return res.status(400).json({ error: "pid required" });
  try { await db.setConsent(pid); res.json({ ok: true }); }
  catch (e) { console.error(e); res.status(500).json({ error: "track failed" }); }
});
app.post("/api/admin/settings", requireAdmin, async (req, res) => {
  const { key, value } = req.body || {};
  if (!key || !["welcome"].includes(key)) return res.status(400).json({ error: "unknown setting" });
  ...
  try {
    ...
    const csv = toCsv(
      ["participant_id", "hotels_liked", "hotels_disliked", "hotels_viewed", "hotels_clicked",
       "avg_seconds_per_hotel", "total_seconds_on_site", "bookmarked_site", "consented", "consent_time", "first_seen", "last_seen"],
      rows
    );
  ...
  try {
    ...
    const csv = toCsv(
      ["participant_id", "hotel_id", "hotel_name", "city", "hotel_rating", "hotel_review_count",
       "list_views", "clicks", "vote", "vote_page", "list_dwell_seconds", "detail_dwell_seconds", "total_dwell_seconds"],
      rows
    );
...
<body><div class="wrap">
  ...
  <div class="tabs">
    ...
    <div class="tab" data-tab="welcome">Welcome text</div>
...
<script>
  ...
  async function loadStats() {
    try {
      ...
      if (!participants.length) { pp.innerHTML = '<div class="empty">No participants yet.</div>'; }
        ...
        pp.innerHTML = '<table><thead><tr><th>Participant</th><th class="n">Liked</th><th class="n">Disliked</th><th class="n">Hotels viewed</th><th class="n">Clicked</th><th class="n">Avg / hotel</th><th class="n">Total time</th><th>Bookmarked</th><th>Agreed</th><th>First seen</th><th>Last seen</th></tr></thead><tbody>' +
          participants.map(function(p){ return '<tr><td><b>'+esc(p.pid)+'</b></td><td class="n up">'+(p.likes||0)+'</td><td class="n down">'+(p.dislikes||0)+'</td><td class="n">'+(p.hotelsSeen||0)+'</td><td class="n">'+(p.hotelsClicked||0)+'</td><td class="n">'+fmtDur(p.avgHotelMs||0)+'</td><td class="n">'+fmtDur(p.totalMs||0)+'</td><td>'+(p.siteFav?'Yes':'No')+'</td><td>'+(p.consentedAt?'Yes':'No')+'</td><td class="muted">'+(p.firstSeen?new Date(p.firstSeen).toLocaleString():'—')+'</td><td class="muted">'+(p.lastSeen?new Date(p.lastSeen).toLocaleString():'—')+'</td></tr>'; }).join('') +
          '</tbody></table>';
      }
  ...
  async function loadWelcome(){
    const r = await fetch('/api/settings/welcome'); const d = await r.json();
    document.getElementById('welcomeText').value = d.text || '';
  }
  async function saveWelcome(value){
    const msg = document.getElementById('welcomeMsg'); msg.textContent = 'Saving…'; msg.style.color = '';
    try {
      const r = await fetch('/api/admin/settings', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ key:'welcome', value }) });
      const d = await r.json(); if (!r.ok) throw new Error(d.error||'Save failed');
      document.getElementById('welcomeText').value = d.value; msg.style.color = '#2E7D5B'; msg.textContent = 'Saved — shown to participants from their next visit.';
    } catch(e) { msg.style.color = '#B3261E'; msg.textContent = e.message; }
  }
