...
import fs from "fs";
import { CITIES as SEED_CITIES } from "./src/cities.js";
...
function withLocalImages(hotels) {
  return hotels.map(h => {
    ...
    const remote = h.image || "";
    const images = REMOTE_FALLBACK ? (h.images || []) : [];   // gallery list also carries remote URLs
...
function withCityCovers(cities) {
  return cities.map(c => {
    ...
    const f = coverId ? localImages.get(coverId) : null;
    if (f) return { ...c, image: `/images/hotels/${f}`, coverHotelId: coverId };
    const own = c.image || "";
