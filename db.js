...
const mem = {
  ...
  participants: {},      // pid -> { firstSeen, lastSeen, totalMs, siteFav, consentedAt }
...
export async function setConsent(pid) {
  if (!pid) return;
  await touchParticipant(pid);
  if (!HAS_DB) { mem.participants[pid].consentedAt = mem.participants[pid].consentedAt || Date.now(); return; }
  await pool.query("UPDATE participants SET consented_at = COALESCE(consented_at, now()) WHERE pid=$1", [pid]);
}
...
export async function participantSummaries() {
  ...
  const parts = await pool.query("SELECT pid,total_ms,site_fav,first_seen,last_seen,consented_at FROM participants");
  ...
  const byPid = {};
  const ensure = pid => (byPid[pid] || (byPid[pid] = { pid, totalMs: 0, siteFav: false, consentedAt: null, likes: 0, dislikes: 0, hotelsSeen: 0, hotelsClicked: 0, avgHotelMs: 0, upvotes: 0, favHotels: [], hotels: [], firstSeen: null, lastSeen: null }));
  for (const p of parts.rows) {
    const o = ensure(p.pid);
    o.totalMs = Number(p.total_ms) || 0; o.siteFav = Boolean(p.site_fav);
    o.firstSeen = p.first_seen; o.lastSeen = p.last_seen; o.consentedAt = p.consented_at;
  }
